...
; Date: 9/25/2020
; Class: CSCE 314
; Description: level-2 function returns all the second level nodes in the tree. 
; second level nodes = all the children of the root (not just the three mentioned in the question)
...
(define (level_2 tree)
  (cond
    ((leaf? tree) '())
    (else (level_2-helper (children tree)))
  )
)

(define (level_2-helper tree)
  (cond
    ((null? tree) '())
    (else (append (list (datum (car tree))) (level_2-helper(cdr tree))))
  )
)

(display (level_2 world-tree2))
(newline)
